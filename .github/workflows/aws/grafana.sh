...
createDashboard() {
  curl -X POST -H "Content-Type: application/json" -d '{"admin":"prom-operator"}' http://admin:prom-operator@localhost:3001/api/orgs
  local AUTH22=$(curl -X POST -H "Content-Type: application/json" -d '{"name":"'"$RANDOM"'", "role": "Admin"}' http://admin:prom-operator@localhost:3001/api/auth/keys)
